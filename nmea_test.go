...
func TestNMEAMessage(t *testing.T) {
	...
	nmeas := []string{
		...
		"$GPDBT,108.34,f,33.02,M,18.06,F*35",
		//"$GPDBT,,,000033.0,M,,*16",
		//"$INDBT,,,000014.5,M,,*06",
		...
		"$GPGGA,091142.234,,,,,0,0,,,M,0.0,M,,0000*5C",
		"$GPTXT,01,01,02,ANTSTATUS=OPEN*2B",
		"$GPRMC,000108.799,V,,,,,0.00,0.00,060180,,,N*4C",
		"$GPRMC,091123.234,V,,,,,,,041217,,,N*41",
		"$GPGSV,1,1,00*79",
	...
	/*
		invalidNmeas := []string{
			// NMEA packet with invalid field(s)
			"$GPRMC,013732.000,A,3150.7238,N,11711.7278,E,0.00,0.00,220013,,,A*6C",
			"$GPRMC,013732.000,A,3150.7238,N,11711.7278,E,0.00,0.00,320213,,,A*6F",
			"$GPRMC,013732.000,A,9150.7238,N,11711.7278,E,0.00,0.00,290200,,,A*6D",
			"$GPRMC,013732.000,A,9150.7238,S,11711.7278,E,0.00,0.00,290200,,,A*6D",
			"$GPRMC,013732.000,A,3150.7238,N,18711.7278,E,0.00,0.00,290200,,,A*6D",
			"$GPRMC,013732.000,A,3150.7238,N,18711.7278,W,0.00,0.00,290200,,,A*6D",
		}

		for _, raw := range invalidNmeas {
			msg, err := Parse(raw)
